...
bool TiXmlDocument::LoadFile(FILE* file, TiXmlEncoding encoding) {
    ...
    */
    ...
    //		* LF:    Multics, Unix and Unix-like systems (GNU/Linux, AIX, Xenix, Mac OS X, FreeBSD, etc.), BeOS, Amiga, RISC
    // OS, and others
...
#ifdef TIXML_USE_STL
void TiXmlDocument::SaveSvgStr(std::string& svgStr) const {
    PrintToStr(svgStr, 0);
}
void TiXmlDocument::PrintToStr(std::string& svgStr, int depth) const {
    for (const TiXmlNode* node = FirstChild(); node; node = node->NextSibling()) {
        node->PrintToStr(svgStr, depth);
        svgStr.append("\n");
    }
}
#endif
